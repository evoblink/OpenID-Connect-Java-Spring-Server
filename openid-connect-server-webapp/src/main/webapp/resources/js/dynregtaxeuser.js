 ...
 *******************************************************************************/
var DynRegTaxeUserModel = Backbone.Model.extend({
    idAttribute: "dynregtaxeuser_id",

    defaults:{
        taxeuser_id:null,
        taxeuser_secret:null,
        redirect_uris:[],
        taxeuser_name:null,
        taxeuser_uri:null,
        logo_uri:null,
        ...
        registration_taxeuser_uri:null
    },
...
var dynRegTaxeUserRootView = Backbone.View.extend({
	...
	render:function() {
    	$(this.el).html($('#tmpl-dynregtaxeuser').html());
    	return this;
	},
	...
	newReg:function(e) {
    	e.preventDefault();
        this.remove();
        app.navigate('dev/dynregtaxeuser/new', {trigger: true});		
	},
	
	editReg:function(e) {
    	e.preventDefault();
		var taxeuserId = $('#taxeuserId').val();
		var token = $('#regtoken').val();
		
		var dynregtaxeuser = new DynRegTaxeUser({
			taxeuser_id: taxeuserId,
			registration_access_token: token
		});
		
		var self = this;
		
		dynregtaxeuser.fetch({success: function() {

    		var userInfo = getUserInfo();
    		var contacts = dynregtaxeuser.get("contacts");
	    	...
	    	var view = new TaxeUserEditView({model: dynregtaxeuser, systemScopeList: app.systemScopeList}); 
	    	
	    	view.load(function() {
	    		$('#content').html(view.render().el);
	    		view.delegateEvents();
	    		setPageTitle("Edit a Dynamically Registered Client");
	    		app.navigate('dev/dynregtaxeuser/edit', {trigger: true});	    		
	    		self.remove();
	    	});
		}, error: function() {
    		$('#modalAlert div.modal-body').html("Invalid dynregtaxeuser or registration access token.");
...
var TaxeUserEditView = Backbone.View.extend({
	...
	initialize:function(options) {
    	this.options = options;
        if (!this.template) {
            this.template = _.template($('#tmpl-dynreg-dynregtaxeuser-form').html());
        }
    ...
    cancel:function(e) {
    	e.preventDefault();
    	app.navigate('dev/dynregtaxeuser', {trigger: true});
    },
    
    deleteClient:function (e) {
    	e.preventDefault();

        if (confirm("Are you sure sure you would like to delete this dynregtaxeuser?")) {
            var self = this;
            ...
            this.model.destroy({
                ...
                error:function (error, response) {
            		console.log("An error occurred when deleting a dynregtaxeuser");
    ...
    toggleClientCredentials:function() {
        ...
        if (tokenEndpointAuthMethod == 'private_key_jwt'
        	|| tokenEndpointAuthMethod == 'taxeuser_secret_jwt') {
    ...
    saveClient:function (e) {
        ...
        this.model.save(attrs, {
            success:function () {
        		...
        		var view = new TaxeUserEditView({model: _self.model, systemScopeList: _self.options.systemScopeList});
    ...
    render:function() {
		$(this.el).html(this.template({dynregtaxeuser: this.model.toJSON(), userInfo: getUserInfo()}));
        ...
        this.$('.nyi').clickover({
        	...
        	content: 'The value of this field will be saved with the dynregtaxeuser, '
